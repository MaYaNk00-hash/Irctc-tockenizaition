...
function productionFallbackAudit(tokenId: string): AuditLogEntry[] {
  const now = Date.now();
  return [
    { tokenId, fromStatus: 'WAITING_ROOM', toStatus: 'ADMITTED', reason: 'Booking request admitted to the Tatkal queue.', createdAt: new Date(now - 2000) },
    { tokenId, fromStatus: 'ADMITTED', toStatus: 'SEAT_LOCKED', reason: 'Selected seat was temporarily locked for this booking.', createdAt: new Date(now - 1000) },
    { tokenId, fromStatus: 'SEAT_LOCKED', toStatus: 'CONFIRMED', reason: 'Booking confirmation generated by the production demo flow.', createdAt: new Date(now) }
  ];
}
...
export class AuditService {
  ...
  public static async getAuditHistoryWithSource(tokenId: string): Promise<{ entries: AuditLogEntry[]; source: 'postgres' | 'redis' | 'demo' | 'fallback' | 'unavailable' }> {
    ...
    return { entries: productionFallbackAudit(tokenId), source: 'fallback' };
  }
